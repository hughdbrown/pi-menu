...
| `platformer/levels.py` | The sixty level maps, generated by `tools/build_levels.py` | `level` |
| `platformer/bosses.py` | The four demons: their look, and each fight's waves | nothing |
| `pi_menu/music.py` | Chiptune: tuning, tunes, and the player that reads them out | `display.protocol` |
...
Levels are sixteen to thirty-two rows tall; the camera follows in both
directions. Ladders snap the climber into line with the rung — without that a
one-cell hole in a floor is impassable to a one-cell player, and every tall
level's first draft was unwinnable for exactly that reason.
...
Levels 15, 30, 45 and 60 end under an eight-by-eight demon drawn dim *behind*
the level — at a quarter of the panel it can only work as scenery, with the
fight in front of it. It slams three-cell fists down where the player stands
and throws fireballs on a script; both kill on touch. There is no health bar:
the fight is a fixed set of waves, each faster, and surviving to the end is
what makes the demon withdraw and opens the goal.

The fists are the only reactive thing in the game, and they aim once, at
launch, at even columns only — which is what keeps a boss level verifiable.
Everything else, fireballs included, is a function of the clock. The solver
carries `boss_tick` whole (a fight is a script with an end, not a cycle) and
banded costs make it finish one job before the next: coins, then the fight,
then the goal.

## Music

Three synth channels on the Pico — square lead, triangle bass, noise drums —
driven the same way as the pixels: the Pi decides every note and sends it as
it falls due (protocol v3, `CMD_TONE`/`CMD_HUSH`). Tunes are three strings of
note names read in step. Frames and notes queue differently in the pump: a
dropped frame is one you never see, a dropped note-off is a sound that never
stops. Seven original tunes; the finale for level sixty is an original piece
in an eighties style, deliberately not a transcription of any real song.
