...
def test_the_hotbar_runs_along_the_bottom_with_the_selected_slot_blinking(scene):
    ...
    assert tile_pixels(canvas, 7, 7) == (tl, tr, bl, br) == (render.BLACK,) * 4  # slot 8 is empty
    later = Canvas()
...
def test_the_inventory_screen_has_its_buttons_where_the_hit_test_says():
    ...
    assert canvas.get(0, 0) == palette.slot_colour(0, 0) == render.BLACK  # an empty armour slot
